...
const struct linklist CWtree[67] = {
    ...
    {"3", 63, 63},    // 34
    {
        ...
        53,
        63,
    },                // 35 * used for all unidentifiable characters ¬
    {"2", 63, 63},    // 36
    {"<as>", 63, 63}, // 37 !! <as>
    {"*", 54, 63},    // 38
    {"+", 63, 55},    // 39
...
//// we define two classes: MorseTable (a pointer to the linked list defined
/// above, and mthods to manipulate the pointer) /                   and Decoder
///(which decodes signals from a key or from audio - derived from a Goertzel
/// filter)
...
private:
  std::function<void(bool)> drawInputStatusCb;
  std::function<void(String)> displayMorseCb;

  boolean fromKey; /// if true, decode  from straight key, if false, from audio
                   /// through Goertzel
  ...
  unsigned int nbtime; /// ms noise blanker
  ...
  long startTimeLow;
  long lowDuration;
  unsigned long ditAvg,
      dahAvg; /// average values of dit and dah lengths to decode as dit or dah
              /// and to adapt to speed change
  ...
  // byte treeptr;                          // pointer used to navigate within
  // the linked list representing the dichotomic tree
  MorseTable myTable;

public:
  Decoder(std::function<void(bool)> onInputChange,
          std::function<void(String)> displayMorseCb);
  void setup();
  void decode();
  uint8_t getWpm();
